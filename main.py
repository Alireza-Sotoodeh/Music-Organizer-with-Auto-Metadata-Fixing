...
Features:
1. Interactive prompts for root & output folders, copy/move mode, format conversion.
...
8. Convert all tracks to a single format (via pydub/ffmpeg) or keep original.
9. Copies or moves processed files to an `output` folder, structured by Artist/Title–Album.
10. Logs to console (Rich), to ~/.music_organizer/organizer.log, and to ~/.music_organizer/report.csv.
"""
...
# WAV tagging is limited; mutagen.wave.WAVE can read only
...
# Genius® API token (from env or config)
GENIUS_TOKEN = (
...
def extract_existing_cover(path: Path) -> Optional[bytes]:
    ...
    try:
        if suf == ".mp3":
            ...
            if covrs:
                return covrs[0]
        # .ogg/.wav: usually no embedded cover
    except Exception:
        return None
    return None
...
def embed_tags_and_cover(
    ...
    """Write tags+lyrics+cover back into file."""
    suf = path.suffix.lower()

    # If no cover, try iTunes
...
def convert_audio(
    ...
    """
    Uses pydub/ffmpeg to convert `src` to `dst` with format=target_fmt.
    """
    src_fmt = src.suffix.lower().lstrip(".")
    audio = AudioSegment.from_file(str(src), format=src_fmt)
    audio.export(str(dst), format=target_fmt)

# -----------------------------------------------------------------------------
# SECTION 6: Process One File
# -----------------------------------------------------------------------------
def process_file(
    src: Path,
    conn,
    ...
    suffix   = convert_to or src.suffix.lower().lstrip(".")
    ext      = "." + suffix

    if alb_fn:
        new_name = f"{tit_fn} – {alb_fn}{ext}"
    else:
        new_name = f"{tit_fn}{ext}"

    dest_dir  = output_root / art_dir
    dest_dir.mkdir(parents=True, exist_ok=True)
    dest_path = dest_dir / new_name

    # Copy or Move or Convert
    action = ""
    if convert_to and convert_to != src.suffix.lower().lstrip("."):
        console.log(f"[cyan]Converting[/cyan] {src.name} → {new_name}")
        if copy_mode:
            convert_audio(src, dest_path, convert_to)
            action = f"CONVERT→COPY"
        else:
            convert_audio(src, dest_path, convert_to)
            src.unlink(missing_ok=True)
            action = f"CONVERT→MOVE"
    else:
        # no conversion
        if copy_mode:
...
def main():
    ...
    # 4) Format conversion
    conv_choices = ["keep","mp3","flac","wav","aac"]
    conv = Prompt.ask("Convert all tracks to:", choices=conv_choices, default="keep")
    convert_to = None if conv=="keep" else conv
    ...
    # 8) Process with Rich Progress
    console.print(Panel.fit(
        f"🎵 Organizer\nRoot: [bold]{root}[/bold]\nOutput: [bold]{output_root}[/bold]\n"
        f"Mode: [bold]{mode}[/bold]  Convert→ [bold]{conv}[/bold]\nFiles: [bold]{total}[/bold]\n",
    ...
    with Progress(
        ...
        for src in all_files:
            ...
            process_file(
                ...
                output_root, copy_mode, convert_to, network_ok
            )
